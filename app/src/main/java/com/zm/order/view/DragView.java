...
public class DragView extends ListView{
	...
	public DragView(Context context, AttributeSet attrs) {
		...
		Log.e("dragview","最小距离="+scaledTouchSlop);
	}
...
//	重写于absListView
		...
		public boolean onInterceptTouchEvent(MotionEvent ev) {
			...
			{
				Log.e("test","onInterceptTouchEvent down");
				...
				Log.e("test","dragSrcPosition = dragPosition="+dragSrcPosition);
				//如果点击在列表之外，也就是不允许的位置
				...
				Log.e("test","dragPoint="+dragPoint+"dragOffset="+dragOffset);
				//拿到拖动的imageview对象
				//View drager = itemView.findViewById(R.id.iv_caipin);
				...
				/*if(drager != null && x<drager.getRight()-20){
					...
					Log.e("test","upScrollBounce="+upScrollBounce+"downScrollBounce="+downScrollBounce);
			...
			{
				Log.e("test","onInterceptTouchEvent MOVE");
			}
			return super.onInterceptTouchEvent(ev);
		}
...
//	        }
	       ...
	       //数据交换
	        if(dragPosition>=0&&dragPosition<getAdapter().getCount())
	        {
	            ...
	            Log.e("test","dragSrcPosition="+dragSrcPosition+"dragPosition="+dragPosition);
//	            //原始位置的item
	            if(dragSrcPosition!=dragPosition)
	            {

					OrderDragAdapter adapter = (OrderDragAdapter)getAdapter();
					adapter.insert(dragSrcPosition,dragPosition);
					Document kindDocument=adapter.getKindDocument();
					try {
					/*	kindDocument.update(new Document.DocumentUpdater() {
							@Override
							public boolean update(UnsavedRevision newRevision) {
								Map<String, Object> properties = newRevision.getUserProperties();
								List<String> list= (List<String>) properties.get("dishesListId");
								String id=list.get(dragPosition);
								list.set(dragPosition,list.get(dragSrcPosition));
								list.set(dragSrcPosition,id);
								newRevision.setUserProperties(properties);
								return false;
							}
						});*/
						Map<String, Object> properties = kindDocument.toMap();
						List<String> list= (List<String>) properties.get("dishesListId");
						String id=list.get(dragPosition);
						list.set(dragPosition,list.get(dragSrcPosition));
						list.set(dragSrcPosition,id);
						CDBHelper.saveDocument(getContext(),kindDocument);
					} catch (Exception e) {
						e.printStackTrace();
					}
